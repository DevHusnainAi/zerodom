...
export class InteractionGraph {
  ...
  toCompactText(options: { selectors?: boolean; hrefs?: boolean } = {}): string {
    ...
    for (const node of this.nodes) lines.push(compactLine(node, options.selectors, options.hrefs));
    return lines.join("\n");
  }
}
...
export class ZeroDOMParser {
   ...
   */
  private walk(): { found: Array<[Element, Element | null]>; labelsByFor: Map<string, Element> } {
    const found: Array<[Element, Element | null]> = [];
    const labelsByFor = new Map<string, Element>();
    // [element, nearest ancestor <label>], pushed reversed so pops stay in doc order.
    const stack: Array<[Element, Element | null]> = [[this.root, null]];
    ...
    while (stack.length) {
      ...
      if (ZeroDOMParser.isInteractive(el)) found.push([el, ancestorLabel]);

      const children = Array.from(el.children);
      for (let i = children.length - 1; i >= 0; i--) stack.push([children[i], ancestorLabel]);
    }
...
export function parseHtml(html: string, url = "about:blank"): InteractionGraph {
