...
- **`cli.py`** — `zerodom inspect`/`audit`/`relay`/`scan`/`crawl`/`compare`/`extension`/`setup`; `--pipe` (JSONL nodes), `--stealth`, `-` reads target URLs from stdin. Engagement flags on inspect+scan: `--proxy`/`$ZERODOM_PROXY`+`--insecure` (Burp/Caido), `--header 'K: V'` (repeatable), `--storage-state` (reuse a cleared session). `--frames`+`--stealth` errors; `_goto` bounds `networkidle` so challenge pages don't hang. `zerodom compare URL --as NAME=STATE.json` (≥2): fetch under each identity's cookies, diff responses (byte-identical = cross-tenant IDOR). `zerodom crawl URL`: rendered, scoped, read-only BFS recon → attack-surface map (forms, links, fired API calls) as JSONL. `zerodom setup`: guided first-run — Chromium check, extension path, MCP config, relay reachability (print-and-guide, no account).
